...
const width = 500, height = 750, entities = []
		let lastTime = performance.now(), delta = 0, gameStart = false
		...
		class Paddle {
			...
			update(delta) {
				this.x = Math.max(0, Math.min(this.x + this.moveSpeed * this.moveDir * delta, this.bound))
			}
		...
		class Ball extends EventTarget {
			...
			randomDir(side) {
				...
				this.moveDir = (Math.PI * 0.60 * Math.random() - Math.PI * 0.2) + (side == 1 ? 0 : Math.PI)
			}
			...
			checkCollision(entities) {
				entities.forEach((entity) => {
					...
					const col = entity.getPaddleCollider()
						if (this.lastCollide != col && (this.x > col.x - this.radius && this.x < col.x + col.paddleWidth + this.radius && this.y > col.y - this.radius && this.y < col.y + col.paddleHeight + this.radius)) {
							this.lastCollide = col
							this.moveSpeedX = Math.max(100, Math.min(350, this.moveSpeedX * Math.random() * 2))
							this.moveSpeedY = Math.max(100, Math.min(350, this.moveSpeedY * Math.random() * 2))
		...
		class CPUController {
			...
			checkSide() {
				this.canMove =  this.balls.reduce((check, ball) => check || ball.side == this.side, false)
				
				//console.error(this.canMove)
			}
			...
			update(delta) {
				const ball = this.balls.filter(ball => ball.side == this.side).sort((ballA, ballB) => this.distTo(ballA) - this.distTo(ballB))[0]
		...
		class PlayerController {
			constructor(paddle, bound, side) {
				...
				document.body.addEventListener("touchend", this.handleInput.bind(this, null))
			...
			handleInput(e) {
				...
				if (event)
					this.paddle.moveDir = event.clientX >= this.bound ? 1 : -1
				else
					this.paddle.moveDir = 0
			}
		...
		function render() {
			...
			context.fillText("CPU: " +  playerCPU.score, (width - size.width) * 0.5, height * 0.25 - fontSize * 0.5)
			
			size = context.measureText("YOU: " + player.score)
			context.fillText("YOU: " + player.score, (width - size.width) * 0.5, height * 0.75 + fontSize * 0.5)
		...
		const paddle1 = new Paddle(width / 2, height * 0.95, width), paddle2 = new Paddle(width * 0.5, height * 0.05, width), ball1 = new Ball(width * 0.25, height * 0.5, 10, width, height), ball2 = new Ball(width * 0.75, height * 0.5, 10, width, height), ball3 = new Ball(width * 0.5, height * 0.5, 10, width, height), playerCPU = new CPUController([ball1, ball2, ball3], paddle2, 0), player = new PlayerController(paddle1, width / 2, 1)
		
		entities.push(player, playerCPU, ball1, ball2, ball3)
